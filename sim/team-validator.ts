...
export class TeamValidator {
	...
	validateSet(set: PokemonSet, teamHas: AnyObject): string[] | null {
		...
		if (template.maleOnlyHidden && setSources.sourcesBefore < 5 && setSources.sources.every(source => source.charAt(1) === 'E')) {
		...
		while (template && template.species && !alreadyChecked[template.speciesid]) {
			...
			if (template.learnset[moveid] || template.learnset['sketch']) {
				...
				for (let learned of lset) {
					...
					if (learnedGen < 7 && setSources.isHidden && !dex.mod('gen' + learnedGen).getTemplate(template.species).abilities['H']) {
